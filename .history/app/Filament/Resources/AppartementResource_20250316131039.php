        ...
        return $table
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('numappartement')
                    ->label('Client')
                    ->options(fn () => Client::pluck('numappartement', 'numappartement')->toArray())
                    ->searchable()
                    ->preload(),                
                // Bloc Appartement - Case-Insensitive Search with Active Filter Display
                Filter::make('blocappartement')
                    ->form([
                        \Filament\Forms\Components\TextInput::make('value')
                            ->label('Bloc Appartement')
                            ->placeholder('Enter Bloc')
                    ])
                    ->query(fn (Builder $query, array $data) => 
                        isset($data['value']) && $data['value'] !== ''
                            ? $query->whereRaw('LOWER(blocappartement) LIKE ?', ['%' . strtolower($data['value']) . '%'])
                            : $query
                    )
                ...
                // Étage - Case-Insensitive Search with Active Filter Display
                Filter::make('etage')
                    ->form([
                        \Filament\Forms\Components\TextInput::make('value')
                            ->label('Étage')
                            ->placeholder('Enter Étage')
                    ])
                    ->query(fn (Builder $query, array $data) => 
                        isset($data['value']) && $data['value'] !== ''
                            ? $query->whereRaw('LOWER(etage) LIKE ?', ['%' . strtolower($data['value']) . '%'])
                            : $query
                    )
                    ->indicateUsing(fn (array $data) => 
                        isset($data['value']) && $data['value'] !== ''
