        ...
        return $form
            ->schema([
                ...
                Section::make('Indication Temporelle')->schema([
                    ...
                    ->relationship('lastModifiedBy', 'id') // Establishes the relationship
                    ->getOptionLabelFromRecordUsing(fn ($record) => "{$record->name}") // Displays the user's name instead of ID
                    ->searchable()
                    ->preload()
                    ->disabled(),
                    ...
                    ->label('Supprimé par')
                    ->relationship('deletedBy', 'id') // Establishes the relationship
                    ->getOptionLabelFromRecordUsing(fn ($record) => "{$record->name}") // Displays the user's name instead of ID
                    ->searchable()
                    ->preload()
                    ->disabled()
