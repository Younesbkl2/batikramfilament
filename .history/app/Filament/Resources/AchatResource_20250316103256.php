...
class AchatResource extends Resource
{
    ...
    {
        $totalCount = static::getModel()::count(); // Always show total count
        $trashedCount = static::getModel()::onlyTrashed()->count(); // Soft-deleted count
    
        // If there are soft-deleted entries, show both badges
        return $trashedCount > 0 ? "{$totalCount} <span style='background-color: red; color: white; padding: 2px 6px; border-radius: 5px;'>{$trashedCount}</span>" : (string) $totalCount;
    }
    
    public static function getNavigationBadgeColor(): ?string
    {
        return 'primary'; // Keeps the main badge color blue
    }
