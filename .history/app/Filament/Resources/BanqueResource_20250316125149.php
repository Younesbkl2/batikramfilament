    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->filters([
                ...
                // nomdebanque - Case-Insensitive Search with Active Filter Display
                Filter::make('nomdebanque')
                    ->form([
                    ...
                    ->query(fn (Builder $query, array $data) => 
                        isset($data['value']) && $data['value'] !== ''
                            ? $query->whereRaw('LOWER(nomdebanque) LIKE ?', ['%' . strtolower($data['value']) . '%'])
                            : $query
                    )
                    ->indicateUsing(fn (array $data) => 
                        isset($data['value']) && $data['value'] !== ''
                            ? 'Nom de la Banque: ' . $data['value'] 
                            : null
                    ),
