...
class ProduitResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('codeprod')->sortable()->searchable(),
                Tables\Columns\TextColumn::make('Typeproduit')->sortable()->searchable(),
            ...
            ->filters([
                ...
                // codeprod - Case-Insensitive Search with Active Filter Display
                Filter::make('Typeproduit')
                    ->form([
                        \Filament\Forms\Components\TextInput::make('value')
                            ->label('Type de Produit')
                            ->placeholder('Enter le Type de Produit')
                    ])
                    ->query(fn (Builder $query, array $data) => 
                        isset($data['value']) && $data['value'] !== ''
                            ? $query->whereRaw('LOWER(Typeproduit) LIKE ?', ['%' . strtolower($data['value']) . '%'])
                            : $query
                    )
                    ->indicateUsing(fn (array $data) => 
                        isset($data['value']) && $data['value'] !== ''
                            ? 'Type de Produit: ' . $data['value'] 
                            : null
                    ),
