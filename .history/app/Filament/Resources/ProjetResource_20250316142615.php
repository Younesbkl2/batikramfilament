    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->filters([
                ...
                // Libelleprj - Case-Insensitive Search with Active Filter Display
                Filter::make('Libelleprj')
                    ->form([
                        \Filament\Forms\Components\TextInput::make('value')
                            ->label('Libelle du Projet')
                            ->placeholder('Enter le Libelle du Projet')
                    ])
                    ->query(fn (Builder $query, array $data) => 
                        isset($data['value']) && $data['value'] !== ''
                            ? $query->whereRaw('LOWER(Libelleprj) LIKE ?', ['%' . strtolower($data['value']) . '%'])
                            : $query
                    )
                    ->indicateUsing(fn (array $data) => 
                        isset($data['value']) && $data['value'] !== ''
                            ? 'Libelle du Projet: ' . $data['value'] 
                            : null
                    ),
