    ...
    protected function getFilters(): ?array
    {
        return [
            'year' => collect(range(now()->year - 5, now()->year))
                ->mapWithKeys(fn ($year) => [(string) $year => (string) $year]) // Ensure keys & values are strings
                ->toArray(),
    
            'modepaie' => Paiement::distinct()
                ->whereNotNull('modepaie') // Remove null values
                ->pluck('modepaie')
                ->mapWithKeys(fn ($mode) => [(string) $mode => (string) $mode]) // Ensure string format
                ->toArray(),
        ];
    }
