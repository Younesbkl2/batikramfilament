    ...
    protected function getFilters(): ?array
    {
        return [
            'year' => collect(range(now()->year - 5, now()->year))
                ->mapWithKeys(fn ($year) => [$year => (string) $year]) // Ensure values are strings
                ->toArray(),
            'modepaie' => Paiement::distinct()
                ->whereNotNull('modepaie') // Avoid null values
                ->pluck('modepaie', 'modepaie')
                ->map(fn ($value) => (string) $value) // Ensure values are strings
                ->toArray(),
        ];
    }
