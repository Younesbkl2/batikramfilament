    ...
    protected function getFilters(): ?array
    {
        return [
            'year' => collect(range(now()->year - 5, now()->year))
                ->mapWithKeys(fn ($year) => [(string) $year => (string) $year]) // Ensure keys and values are strings
                ->toArray(),
            'modepaie' => Paiement::distinct()->pluck('modepaie')->filter()
                ->mapWithKeys(fn ($mode) => [(string) $mode => (string) $mode]) // Ensure associative array
                ->toArray(),
        ];
    }
