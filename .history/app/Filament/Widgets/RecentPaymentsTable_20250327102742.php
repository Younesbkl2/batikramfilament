...
{
    use HasTabs; // Use the HasTabs trait to enable tab functionality
    ...
    protected function getTabs(): array
    {
        return [
            'this_week' => Tab::make('Cette Semaine')
                ->modifyQueryUsing(fn (Builder $query) => $query->whereBetween('datepaie', [now()->startOfWeek(), now()->endOfWeek()])),

            'this_month' => Tab::make('Ce Mois')
                ->modifyQueryUsing(fn (Builder $query) => $query->whereBetween('datepaie', [now()->startOfMonth(), now()->endOfMonth()])),

            'this_year' => Tab::make('Cette Année')
                ->modifyQueryUsing(fn (Builder $query) => $query->whereBetween('datepaie', [now()->startOfYear(), now()->endOfYear()])),
        ];
    }
