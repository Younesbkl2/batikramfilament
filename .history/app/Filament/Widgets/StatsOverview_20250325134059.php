    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make('Total dépense Appartement', $this->formatDzd($totalDepenseAppartement))
                ...
                ->extraAttributes(['class' => 'compact-stat']),
            
            Stat::make('Montant total restant Appartement (réservé)', $this->formatDzd($totalRestantAppartement))
                ->color($totalRestantAppartement > 0 ? 'danger' : 'success')
                ->chart([7, 2, 10, 3, 15, 4, 17])
                ->extraAttributes(['class' => 'compact-stat']),
            
            Stat::make('Total a payer Appartement (réservé)', $this->formatDzd($totalAPayerAppartement))
                ->color('primary')
                ->chart([7, 2, 10, 3, 15, 4, 17])
                ->extraAttributes(['class' => 'compact-stat']),
