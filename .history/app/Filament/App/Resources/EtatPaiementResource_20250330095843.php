...
use App\Filament\Exports\EtatPaiementExporter;
...
class EtatPaiementResource extends Resource
{
            ...
            ->schema([
                ...
                Section::make('Résumé financier')
                    ->schema([
                        ...
                        TextInput::make('pourcentage_paye')
                            ->label('% Payé')
                            ->numeric()
                            ->disabled()
                            ->suffix('%'),
                    ...
                    ->columns(4), // Changed from 3 to 4 columns
   ...
   public static function table(Table $table): Table
   {
       return $table
           ...
           ->columns([
               ...
               TextColumn::make('prix_local')
                ...
                TextColumn::make('reste_a_payer')
                    ...
                    ->sortable()
                    ->toggleable()
                    ->color(fn ($record) => $record->pourcentage_paye < 100 ? 'danger' : 'success'),
                TextColumn::make('pourcentage_paye')
                    ->label('% Payé')
                    ->numeric(
                        decimalPlaces: 2,
                    )
                    ->suffix('%')
                    ->color(fn ($record) => $record->pourcentage_paye >= 100 ? 'success' : 'danger')
                    ->badge()
                    ->sortable()
                    ->toggleable(),
           ...
           ->headerActions([
            ExportAction::make()->label('Exporter tous les Etats de Paiements')->exporter(EtatPaiementExporter::class),
        ])
           ->bulkActions([
            ExportBulkAction::make()->label('Exporter les Etats Paiements sélectionnés')->exporter(EtatPaiementExporter::class)
           ]);
   }
