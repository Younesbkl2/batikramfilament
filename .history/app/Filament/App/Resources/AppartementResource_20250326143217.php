...
use App\Filament\Exports\AppartementExporter;
use Filament\Tables\Actions\ExportBulkAction;
use Illuminate\Database\Eloquent\SoftDeletingScope;
use App\Filament\App\Resources\AppartementResource\Pages;
use App\Filament\App\Resources\AppartementResource\RelationManagers;
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                Forms\Components\TextInput::make('NumEDD')->label('Num_EDD'),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                ->formatStateUsing(fn ($record) => "{$record->codeprj} ({$record->projet->Libelleprj} {$record->projet->adresseprj})")
                ->sortable()
                ->toggleable(),
                ...
                ->sortable()
                ->toggleable(),
                Tables\Columns\TextColumn::make('NumEDD')->label('Num_EDD')->sortable()->searchable()->toggleable(),
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('blocappartement')
                    ...
                    ->searchable()
                    ->preload()
                    ->options(fn () => Appartement::whereNotNull('blocappartement')->pluck('blocappartement', 'blocappartement')->toArray()),
                Tables\Filters\SelectFilter::make('superficie')
                    ->label('Superficie')
                    ->searchable()
                    ->preload()
                    ->options(fn () => Appartement::whereNotNull('superficie')->orderBy('superficie')->pluck('superficie', 'superficie')->toArray()),                      
                Tables\Filters\SelectFilter::make('etage')
                    ->label('Etage')
                    ->searchable()
                    ->preload()
                    ->options(fn () => Appartement::whereNotNull('etage')->pluck('etage', 'etage')->toArray()),            
            ...
            ->headerActions([
                ExportAction::make()->label('Exporter tous les Appartements')->exporter(AppartementExporter::class),
            ])
            ->bulkActions([
                ...
                ExportBulkAction::make()->label('Exporter les Appartements sélectionnés')->exporter(AppartementExporter::class)
            ]);
    }
